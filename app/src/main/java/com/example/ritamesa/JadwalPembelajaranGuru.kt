...
class JadwalPembelajaranGuru : AppCompatActivity() {
    ...
    private fun addNewClass(className: String) {
        ...
        // Buat Request Body
        val request = com.example.ritamesa.data.model.CreateClassRequest(
            grade = grade,
            label = label,
    ...
    private fun updateClass(classId: Int, newName: String) {
        ...
        // Buat ClassItem dengan data baru
        val updatedClass = ClassItem(
            id = classId,
            name = newName,
            scheduleImagePath = null // Tetap sama, atau bisa juga di-update
        )

        apiService.updateClass(classId, updatedClass).enqueue(object : Callback<GeneralResponse> {
            override fun onResponse(call: Call<GeneralResponse>, response: Response<GeneralResponse>) {
                if (response.isSuccessful) {
                    Toast.makeText(this@JadwalPembelajaranGuru, "Kelas berhasil diupdate", Toast.LENGTH_SHORT).show()
                    loadClassesFromApi()
