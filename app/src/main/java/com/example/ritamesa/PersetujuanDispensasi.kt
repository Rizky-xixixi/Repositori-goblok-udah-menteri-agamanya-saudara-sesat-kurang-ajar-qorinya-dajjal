...
class PersetujuanDispensasi : AppCompatActivity() {
                ...
                if (response.isSuccessful) {
                    ...
                    data.forEach { item ->
                        ...
                        allDispensasiList.add(Dispensasi(
                            ...
                            kelas = "-", // Extract from schedule or student info if available
                            mataPelajaran = item.schedule?.subjectInfo?.name ?: "-",
                            hari = "-", // Parse from date if needed
                            tanggal = item.date,
                            jamKe = if (item.schedule != null) "${item.schedule.startTime} - ${item.schedule.endTime}" else "-",
    ...
    private fun processApproval(dispensasi: Dispensasi, isApproved: Boolean, dialog: Dialog) {
        ...
        val call = if (isApproved) {
            apiService.approveAbsenceRequest(dispensasi.id)
        } else {
            apiService.rejectAbsenceRequest(dispensasi.id)
        }
